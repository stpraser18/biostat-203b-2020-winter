...
**Solution:** The last command will print five lines. The head -n is reading the beginning lines two lines, which is a blank and then author. The tail reads the last three lines from the bottom.The first line of the shell script is necessary to ensure that the "sh" as the parent to execute any further commands.
...
```{bash eval=TRUE}
Rscript autoSim.R seed=203 reps=50
```
