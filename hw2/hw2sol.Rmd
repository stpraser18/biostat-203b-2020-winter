...
rowid_subjectid <- inner_join(ditems_row, ascend_time, by = "ROW_ID") %>%
  select(ROW_ID, SUBJECT_ID, ITEMID, LABEL)
rowid_subjectid 
```
...
final_within <- dod_intime %>%
  mutate(within_days = as.numeric(as.duration(dateofdeath - intime_date) / 86400)) %>%
  drop_na() %>%
  filter(within_days < 30)
final_within
```
