...
#Extracted itemid and value from chartevents
```{r}
...
#Merging chartevents and d_items together
```{r}
mergechartitems <- left_join(chartevents, d_items)
mergechartitems
```

#Find systolic strings in data
```{r}
mergechartitems2 <- mergechartitems %>%
  select(LABEL)
  mergechartitems2
```
#Unique subjects
#```{r}
unisubj_id <- read_csv("/home/203bdata/mimic-iii/CHARTEVENTS.csv") %>%
  select(SUBJECT_ID) %>%
  distinct(SUBJECT_ID)
unisubj_id
```
#Joined unique subjects, itemid, and value into one table
#```{r}
right_join(unisubj_id, chartevents)
```
...
#unique patients from icustays
```{r}
firsticu_unique <- icustays %>%
  distinct(SUBJECT_ID) %>%
  select(SUBJECT_ID)
  firsticu_unique
```

#select first care unit
```{r}
firsticu <- icustays %>%
...
#merged unique patients and first care unit
```{r}
joinu_ficu <- left_join(firsticu_unique, firsticu)
joinu_ficu
```

#unique patients in admissions
```{r}
admissions_subj <- admissions %>%
  select(SUBJECT_ID) %>%
  distinct(SUBJECT_ID)
admissions_subj
```
