...
#Try to plot the US map with coronavirus
```{r}
plotdate <- "2020-02-14"
case <- "confirmed"
us_map %>%
  filter(`Province/State` %in% c("Boston,MA", "Chicago, IL", "Humboldt County, CA", "Los Angeles, CA")) %>%
  filter(Date == plotdate, Case == case) %>%
  group_by(`Province/State`) %>%  
  top_n(1, Date) %>%
  right_join(chn_prov, by = "Province/State") 
```
