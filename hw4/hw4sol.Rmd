...
This logistic regression is done to see how multiple variables affect mortality. The second logistic regression that is done after this one looks at just seq_num and age, which pertains to the hypothesis.
```{r}
...
```{r}
logistic_seq_num <- glm(mortality ~ seq_num, data = study_admissionsfull,
                     family = binomial(link = "logit"), maxit = 100)
```
...
```
```{r}
plot(logistic_respmort) 
set.seed(1234)
(logistic_respmort <- glm(sample(c(0, 1), size=100,
                  replace=TRUE, prob=c(0.5, 0.5)) ~ 0 
...
```{r}
fit = glm(mortality ~ seq_num, data = study_admissionsfull,
                     family = binomial(link = "logit"), maxit = 100)
newdat <- data.frame(seq_num = seq(min(1), max(5),len=100))
newdat$mortality = predict(fit, newdata = newdat, type="response")
plot(age, mortality, data = study_admissionfull, col="red4")
lines(mortality ~ age, newdat, col="green4", lwd=2)
```
```{r}
mydf <- merge(study_admissionsfull, probs, by="position")

```
Plotting Logistic Regression 
```{r}
b0 <- logistic_respmort$coef[1] # intercept
age <- logistic_respmort$coef[2]
seq_num <- logistic_respmort$coef[3]
genderM <- logistic_respmort$coef[4]
ethnic_groupBLACK <- logistic_respmort$coef[5]
ethnic_groupHISPANIC <- logistic_respmort$coef[6]
ethnic_groupUNKNOWN <- logistic_respmort$coef[7]
ethnic_groupWHITE <- logistic_respmort$coef[8]
principal_dxTRUE <- logistic_respmort$coef[9]

range <- seq( 0, 10, .01 )

logodds_var_1 <- b0 + age * range
logodds_var_2 <- b0 + seq_num * range
logodds_var_3 <- b0 + genderM * range
logodds_var_4 <- b0 + ethnic_groupBLACK * range
logodds_var_5 <- b0 + ethnic_groupHISPANIC * range
logodds_var_6 <- b0 + ethnic_groupUNKNOWN * range
logodds_var_7 <- b0 + ethnic_groupWHITE * range
logodds_var_8 <- b0 + principal_dxTRUE * range

prob_var_1 <- exp(logodds_var_1) / (1+exp(logodds_var_1))
prob_var_2 <- exp(logodds_var_2) / (1+exp(logodds_var_2))
prob_var_3 <- exp(logodds_var_3) / (1+exp(logodds_var_3))
prob_var_4 <- exp(logodds_var_4) / (1+exp(logodds_var_4))
prob_var_5 <- exp(logodds_var_5) / (1+exp(logodds_var_5))
prob_var_6 <- exp(logodds_var_6) / (1+exp(logodds_var_6))
prob_var_7 <- exp(logodds_var_7) / (1+exp(logodds_var_7))
prob_var_8 <- exp(logodds_var_8) / (1+exp(logodds_var_8))

plot( x=range, y=prob_var_1, type="l" )
plot( x=range, y=prob_var_2, type="l" )
plot( x=range, y=prob_var_3, type="l" )
plot( x=range, y=prob_var_4, type="l" )
plot( x=range, y=prob_var_5, type="l" )
plot( x=range, y=prob_var_6, type="l" )
plot( x=range, y=prob_var_7, type="l" )
plot( x=range, y=prob_var_8, type="l" )
...
```{r}
age_range <- seq(from=min(0), to=max(100), by=.01)
```
...
dat <- data.frame(
  seq_num =  rbinom(size, 1,5),
  mortality = rbinom(ssize,0,1)
)
...
```{r}
logistic_1 <- glm(mortality ~ seq_num, data = study1, family = binomial(link = "logit"), maxit = 100)
```

I wanted to see if patients that were primarily diagnosed with respiratory illness were related to mortality, but it was not enough to determine the coefficients. 
```{r}
summary(logistic_1)
```
...
#```{r}
set.seed(12345)
train_test_split <- initial_split(study1, prop = 0.6)
train_test_split
```
