...
```{r}
logistic_respmort <- glm(mortality ~ age + seq_num, data = study_admissionsfull, family = binomial(link = "logit"), maxit = 100)
```
...
```{r}
study1 <- glm(mortality ~ seq_num, data = study1, family = "binomial")
```

```{r}
set.seed(12345)

study_1df <- as.data.frame(study_1)
idx <- study_1df$mortality %in% c("mortality")
study_1df <- study_1df[idx,]
y <- ifelse(study_1df$mortality == "mortality", TRUE, NA)
```
...
#```{r}
study1nn <- study1(num_words = max_features)
study$train$x[[1]]
```
