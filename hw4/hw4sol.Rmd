...
Restrict to patients with tuberculous. Found all possible ICD-9 codes related to tuberculous, we search for the string 'tuberculous' in the column long_title from the table d_icd_diagnoses:
```{r}
tbl(con, "d_icd_diagnoses") %>%
  filter(str_detect(tolower(long_title), "tuberculous")) %>%
  print() -> tuberculous_codes
```
...
tbl(con, "diagnoses_icd") %>%
  semi_join(tuberculous_codes, by = "icd9_code") %>%
  print() -> tuberculous_admissions
```
