...
library(ISLR)
```
Let's try to select the patients that have a 30 day mortality rate that were admitted to the MICU primarily from respiratory illness. There is a total of 129 subjects diagnosed with respiratory illness, we have found that subjects have the icd9_code of 51881 which is acute respiratory failure.
```{r}
study_admissions %>%
  group_by(subject_id, hadm_id) %>%
  filter(row_number() == 1) %>%
...
Compute study_1 date which includes all demographics and information of the subjects that were diagnosed with respiratory illness.
```{r}
compute(study_1)
```
Data visualization for the number of 30-day mortality and those that have survived. About 500 patients did not survive. A little less than 400 patients were labeled NA. 
```{r}
study_1 %>%
  group_by(mortality) %>%
  summarize(count = n()) %>%
...
```{r}
study_admissions <- glm(mortality ~ principal_dx, data = study_admissions, family = "binomial")
summary(study_1)
```
...
test <- default[!sample, ]
model <- glm( study_1, family = binomial)
summary(model)$coef
```
...
box_content[2] = "Exclude patients not admitted to ICU \n n= 57,976" # 2nd box
...
5. Analytics. Use at least two analytical approaches to address your research hypothesis/goal. 
**Part 1: Logistic Regression**
```{r}
collect(study_1)
```
