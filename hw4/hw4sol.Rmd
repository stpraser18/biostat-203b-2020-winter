...
study_admissionsfull %>%
  ...
  geom_col()
```
#STILL NEED TO DO THIS drop columns that are NA for mortality to find the true amount of those that did not survive the 30 day mortality 
```{r}
study_admissionsfull %>%
  group_by(mortality) %>%
  drop_na() %>%
  print() -> study_admissionstrue
```
...
print() -> study_1
```
There are 500 deaths after 30 days and there are 390 that are labeled NA. 
```{r}
study_1 %>%
  group_by(mortality) %>%
  summarize(count = n()) %>%
...
```
```{r}
sapply(study_admissionsfull, sd)
```
```{r}
