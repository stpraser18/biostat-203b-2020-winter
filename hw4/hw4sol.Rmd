...
dbListTables(con)
```
#Printing of diagnoses for patients
```{r}
diagnoses <- tbl(con, "diagnoses_icd")
diagnoses %>% print(width = Inf)
```
#Printing icd9_codes
```{r}
icd9 <- tbl(con, "d_icd_diagnoses")
icd9 %>% print(width = Inf)
```
#Join tables for diagnoses and icd9codes for all patients
```{r}
diagnoses %>%
  left_join(icd9, by = c("icd9_code")) %>%
  select(subject_id, hadm_id, seq_num, icd9_code, short_title, long_title) %>%
  print() -> subjects_diagnoses
```
...
Restrict to patients with tuberculous. Found all possible ICD-9 codes related to tuberculosis, we search for the string 'tuberculosis' in the column long_title from the table d_icd_diagnoses:
```{r}
...
`diagnoses_icd` table stores the diagnosis of each admission. We use `semi_join()` to keep the rows in `diagnoses_icd` that match the ICD-9 codes related to tuberculosis:
```{r}
...
tb_admissions %>%
  ...
  print() -> tb_admissions
```
Now we `inner_join` the table of admissions to CCU and the table of admissions that include tuberculosis diagnosis. Very few subjects were admitted to the CCU were diagnosed with tuberculosis.
```{r}
ccu_admissions %>%
...
Created a logical variable indicating if tuberculous is the principal diagonosis or not, whether the seq_num is 1 then it is the principal diagnosis.
