...
2. Introduction. Describe the MIMIC-III data set and what research hypothesis/goal you are to address using this data. **Introduction:** I used the d_icd_diagnoses to obtain the patients who were diagnosed with any respiratory illness and diagnoses_icd to look up the code for the diagnoses. The table of admissions to CCU and table of admissions for the respiratory diagnoses was used. Hypothesis: Patients who were diagnosed with any respiratory illness and admitted into the CCU have a high chance of dying within 30 days as it is one of the top 10 causes of death according to the World Health Organization.
...
resp_admissions %>%
  ...
  print() -> resp_admissions
```
Now we `inner_join` the table of admissions to CCU and the table of admissions that include respiratory diagnosis. 1000 patients got admitted to the CCU that have respiratory illness. 
```{r}
ccu_admissions %>%
...
study_admissions %>%
  ...
  print() -> study_admissions
```
Create graph based on the seq_num, dealing with the primary diagnoses of respiratory illness. 
```{r}
study_admissions %>% 
  group_by(seq_num) %>%
  summarize(count = n()) %>%
...
Let's try to select the patients that have a 30 day mortality rate that were admitted to the CCU primarily from respiratory illness. There is a total of 129 subjects diagnosed with respiratory illness, we have found that subjects have the icd9_code of 51881 which is acute respiratory failure.
```{r}
study_admissions %>%
  group_by(subject_id, hadm_id) %>%
  filter(row_number() == 1) %>%
...
Data visualization for the number of 30-day mortality and those that have survived.
```{r}
study_1 %>%
  group_by(mortality) %>%
  summarize(count = n()) %>%
