...
```{r}
study_fulldf$mortality <- replace(study_admissionsfull$mortality, c("mortality" == 1))
study_fulldf$mortality <- replace(study_admissionsfull$mortality, c("NA" = 0))
head(study_fulldf)
```

```{r}
as.integer(as.logical(study_fulldf$mortality))
head(study_fulldf)
```
...
```{r}
study_admissionsfull$mortality
x_value <- data.frame(seq_num = study_admissionsfull$seq_num)
study_admissionsfull$mortality <- predict(logistic_seq_num, newdata = x_value, type = "response")
ggplot(x_value, aes(x = seq_num, y = study_admissionsfull$mortality)) + geom_line()
```
...
newdf$pout_logit <- predict(logistic_seq_num, newdf, se.fit = TRUE, type = "response")$fit
newdf$pse_logit <- predict(logistic_seq_num, newdf, se.fit = TRUE, type = "response")$se.fit
