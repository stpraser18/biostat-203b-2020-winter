...
1. An informative title. _30-Day Mortality Rate of Tuberculous Patients Admitted to CCU_ 

2. Introduction. Describe the MIMIC-III data set and what research hypothesis/goal you are to address using this data. **Introduction:** I used the d_icd_diagnoses to obtain the patients who were diagnosed with tuberculous and diagnoses_icd to look up the code for the diagnoses. The table of admissions to CCU and table of admissions for the tuberculous diagnoses was used. Hypothesis: Patients who were diagnosed with tuberculous and admitted into the CCU have a high chance of dying within 30 days as it is one of the top 10 causes of death according to the World Health Organization.
...
Restrict to patients with tuberculosis. Found all possible ICD-9 codes related to tuberculosis, we search for the string 'tuberculosis' in the column long_title from the table d_icd_diagnoses. There are 354 codes that deal with tuberculosis. 
```{r}
tbl(con, "d_icd_diagnoses") %>%
  filter(str_detect(tolower(long_title), "tuberculosis")) %>%
  print() -> tb_codes
```

`diagnoses_icd` table stores the diagnosis of each admission. We use `semi_join()` to keep the rows in `diagnoses_icd` that match the ICD-9 codes related to tuberculosis. There are 283 patients that are diagnosed with tuberculosis.
```{r}
tbl(con, "diagnoses_icd") %>%
  semi_join(tb_codes, by = "icd9_code") %>%
  print() -> tb_admissions
```

The `seq_num` field is a priority ranking for the diagnoses generated at the end of stay. In order to focus on patients for whom tuberculosis was central to their hospitalization, we will include records with tuberculosis in any of the first five diagnosis positions, according to the `seq_num` field. To avoid duplicate admissions, we use `group_by()` and `top_n()` to limit the query to the first tuberculosis diagnosis for each admission. Now there are 82 patients that were admitted to hospitalization, with repeated subjects. 
```{r}
tb_admissions %>%
  filter(seq_num <= 5) %>%
  ...
  print() -> tb_admissions
```
Now we `inner_join` the table of admissions to CCU and the table of admissions that include tuberculosis diagnosis. Very few subjects, specifically 3, were admitted to the CCU were diagnosed with tuberculosis, but one patient was primarily admitted with tuberculosis with a seq_num = 1. 
```{r}
ccu_admissions %>%
  inner_join(tb_admissions, by = c("subject_id", "hadm_id")) %>%
  print() -> study_admissions
```

## Transform and augment query tables

Created a logical variable indicating if tuberculous is the principal diagonosis or not, whether the seq_num is 1 then it is the principal diagnosis. Only 1 patient fell under this variable.
...
We want to add information about the severity of patients’ ailments. The `drgcodes` table contains, for `DRG` codes from the All Payers Registry (APR), severity and mortality indicators. We pull the drug severity information and right-join it to our query table. There are 3 patients that are on drugs and the patient that was hospitalized had a drug severity of 4.
```{r}
tbl(con, "drgcodes") %>%
...
Created a new column for the age of death and the mortality column is to see if the death is within 30 days of discharge. Subjects with tuberculosis, age, drug severity, etc...are included. 
```{r}
study_admissions %>%
...
Check to see if any of the drugs that the patients were taking also had HIV.
```{r}
tbl(con, "drgcodes") %>%
  print() -> drgcodes
```
